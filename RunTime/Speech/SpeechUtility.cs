...
namespace HT.Framework.AI
{
    ...
    public static class SpeechUtility
    {
        ...
        private static float[] Convert8BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
        {
            ...
            {
                Debug.LogError("转换AudioClip失败：未获取到有效的WAV字节数据！");
                return null;
            }
        ...
        private static int WriteFileHeader(ref MemoryStream stream, int fileSize)
        {
            ...
            if (count != total)
            {
                Debug.LogError("转换字节数组失败：写入的字节数组长度异常！");
            }
            return count;
        }
        ...
        private static string FormatCode(ushort code)
        {
            switch (code)
            {
                ...
                default:
                    Debug.LogWarning("未知的WAV格式代码:" + code);
                    return "";
            }
        }
        #endregion
        ...
        public static AudioClip ToAudioClip(string filePath)
        {
            ...
            {
                Debug.LogError("加载WAV文件失败：未找到 " + filePath + " 文件！");
                return null;
            }
        ...
        public static AudioClip ToAudioClip(byte[] fileBytes, int offsetSamples = 0, string name = "wav")
        {
            ...
            {
                Debug.LogError("转换AudioClip失败：当前仅支持转换PCM或WAV的扩展格式！");
                return null;
            }
            ...
            switch (bitsPerSample)
            {
                ...
                case 32:
                    ...
                    break;
                default:
                    Debug.LogError("转换AudioClip失败：不支持 " + bitsPerSample + " 量化位数！");
                    return null;
            }
        ...
        public static byte[] FromAudioClip(AudioClip audioClip, bool saveAsFile, string filepath)
        {
            ...
            {
                Debug.LogError("转换字节数组失败：转换完成后的字节数组长度异常！");
                return null;
            }
